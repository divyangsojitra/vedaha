...
function InitPopBox(obj)
{
	...
	if (pbSrc[elem.id] != null || (elem.style.position != "absolute" && elem.style.position != "relative"))
	{
		...
		img.style.borderLeft = "2px solid #8D8D8D";
		img.style.borderRight = "2px solid #8D8D8D";
		img.style.borderBottom = "2px solid #8D8D8D";
		...
		if (bMDone == true && bSDone == true)
		{
			...
			if (popBox[elem.id].isPopped == true)
			{
				...
				if (popBox[elem.id].originalId != null)
				{
					...
					if (navigator.appName == 'Microsoft Internet Explorer') {
					    obj.onclick = function() { Pop(this, 20, '') };

					}
					else {
					    obj.setAttribute("onclick", "Pop(this,20,'');");
					}
...
function CreatePbBar(obj, pbShowBar, pbShowText, pbShowImage, pbText, pbImage, pbBarAbove, isRevert, pbCaption)
{
	...
	if (pbShowImage == true)
	{
		...
		imgPopped.style.left = (left + width - 48) + "px";
...
//		divCapTrans.style.top = top + 20 + "px";
//		divCapTrans.style.backgroundColor = "Gray";
...
//		divCapText.style.top = top + 20+ "px";
...
//		divCapText.style.backgroundColor = "Gray";
//		divCapText.style.color = "#ffffff";
...
function GetPoppedImageSize(obj) 
{
    ...
    if (obj != null && typeof obj.id != 'undefined') {
        ...
        else if (obj.innerWidth && obj.innerHeight) {
...
var popBoxRevertImage = "Images/close-button.jpg";
